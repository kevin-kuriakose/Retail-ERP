...
class POSInvoice(Document):
    ...
    def calculate_totals(self):
        ...
        self.total_tax = tax
        self.grand_total = net + tax - flt(self.loyalty_points_redeemed) * 0.25
    ...
    def _deduct_stock(self):
        ...
        for row in self.items or []:
            create_ledger_entry(
                ...
                voucher_type="POS Invoice", voucher_no=self.name,
                store=self.store, rate=flt(row.rate)
            )
    ...
    def _award_loyalty_points(self):
        if self.customer:
            try:
                ...
                frappe.db.set_value("POS Invoice", self.name, "loyalty_points_earned", pts)
            except Exception:
                frappe.log_error(frappe.get_traceback(), "POS Loyalty Points Error")
